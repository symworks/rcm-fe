...
const CategoryProductTagModal = React.forwardRef(({handleAddFinal, handleUpdateFinal, handleDeleteFinal, ...rest}, ref) => {
  ...
  const validationScheme = yup.object().shape({
    code: yup
    .string()
    .min(1, 'Mã nhãn sản phẩm bắt buộc nhập')
    .max(255, 'Mã nhãn sản phẩm có tối đa 255 ký tự'),
    name: yup
    .string()
    .min(1, 'Tên nhãn sản phẩm bắt buộc nhập')
    .max(255, 'Tên nhãn sản phẩm có tối đa 255 ký tự'),
  });
  ...
  const onSubmit = data => {
    ...
    if (apiType === "insert") {
      ...
      .post(`/api/category_product_tag`, data, {withCredentials: true});
    } else if (apiType === "update") {
      handleFinal = handleUpdateFinal;
      axiosInstance = axiosInstance
      .patch(`/api/category_product_tag`, data, {withCredentials: true});
  ...
  return (
    <div {...rest}>
      <Confirmation
        ref={confirmationRef}
        title="Xóa nhãn sản phẩm"
        detail={
          <span>
            Bạn có muốn chắc xóa nhãn sản phẩm <span className="font-weight-bold">{deleteData?.name}</span> không?
          </span>
        }
...
export default CategoryProductTagModal;
